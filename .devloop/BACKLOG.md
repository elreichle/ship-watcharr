...
- After a container restart the scraping identity's `defaultContact` reports the admin override rather than the compose env value (seen in the 2026-09-10 finish check); the override itself persists, so display only.
