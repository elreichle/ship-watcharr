...
- After a restart the scraping identity's `defaultContact` reports the admin override, not the compose env value (2026-09-10 finish check): `Program.cs` loads `settings.json` over configuration at boot without reload, so `OperatorContactResolver.ResolveDefaultAsync` reads the override as "Configuration", and clearing the override after a restart leaves the old contact in the User-Agent until the next boot.
