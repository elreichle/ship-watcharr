...
- `Ao3ShipIndexScraper.ListingAgreesWithTheLibraryAsync` remarks still describe the pre-T89 schedule: "a request per page per ship per month", "the monthly re-read of every old work's blurb … refresh only when a sweep walks, which it still does whenever the counts differ", and a sweep interval accepting "a fortnight's staleness". Comment only. (T93)
