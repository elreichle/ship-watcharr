...
| D13 | `LastKnownTotalWasAuthenticated` is assigned by whichever run wrote the total, never latched | `:944` | both | the pair travel together or they say something neither run did (T30) | `Does_not_stamp_a_total_it_never_wrote_as_Authenticated`, `Clears_the_Authenticated_flag_when_a_later_run_reads_the_total_anonymously`, `Reads_the_transport_for_whether_the_total_was_Authenticated` — **gap: T44**, the flag ORs across the run while the total is written per page |
...
| F7 | `pagesFetched`, `firstPage` and `lastPage` are set **before** the unreadable break | `:309` | — | **gap: T40**, a run claims to have read a page it could not read |
...
| The authenticated-total flag ORs across a run whose total is written per page | D13 | T44, added to T15's `blocked-by` |
